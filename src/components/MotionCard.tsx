...
function MotionCard({ children }: PropsWithChildren) {
  ...
  const springRotateInteract = useSpringVector({ x: 0, y: 0 }, springInteractSettings);
  const springTranslate = useSpringVector({ x: 0, y: 0 }, springInteractSettings);
  const springRotatePop = useSpringVector({ x: 0, y: 0 }, springPopSettings);
  const springRotateX = useTransform<number, number>(
    [springRotateInteract.x, springRotatePop.x], ([xIn, xPop]) => xIn + xPop
  )
  const springRotateY = useTransform<number, number>(
    [springRotateInteract.y, springRotatePop.y], ([yIn, yPop]) => yIn + yPop
  )
